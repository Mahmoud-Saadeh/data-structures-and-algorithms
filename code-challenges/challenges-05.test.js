...
const toLastNames = people => {
  return people.map((elem)=>`${elem.firstName} ${elem.lastName}`);
};
...
const addPurchases = (arr) => {
  return arr.reduce((total,elem)=>total +=elem.purchasePrice,0 );
};
...
const countNumberOfChildren = (arr) => {
  return arr.reduce(((total, elem)=>total += elem.children ? elem.children.length : 0),0);
};
...
const calculateAverage = (arr) => {
  return arr.reduce((accumulator,elem)=>{
    accumulator.count += 1;
    accumulator.sum += elem;
    accumulator.avgF();
    return accumulator;
  },{ count: 0, sum: 0,avg:0, avgF:function(){this.avg = (this.sum / this.count);} }).avg;
};
...
const extractChildren = (arr) => {
  const filteredArr = arr.filter((elem)=>elem.name.includes('a'));
  return filteredArr.reduce((allChildren,elem)=>{
    if (elem.children) {
      return allChildren.concat(elem.children);
    } else {
      return allChildren;
    }
  },[]);
