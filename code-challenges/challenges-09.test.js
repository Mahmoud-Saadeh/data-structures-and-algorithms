...
const maxInArray = (arr) => {
  return arr.reduce((result,num)=>{
    return result = num > result ? num : result;
  });
};
...
const getHouses = (arr) => {
  let houses = [];
  arr.forEach((elem)=>{houses.push(elem.house);});
  return houses;
};
...
const hasChildrenValues = (arr, character) => {
  ...
  arr.forEach(element => {
    if (element.name === character) {
      let index = Object.keys(element).indexOf('children');
      if(index >= 0){
        result = Object.values(element)[index];
      }
    }
  });
...
const hasChildrenEntries = (arr, character) => {
  ...
  arr.forEach(element => {
    if (element.name === character) {
      let index = Object.keys(element).indexOf('children');
      if(index >= 0){
        result = Object.entries(element)[index];
      }
    }
  });
  return result.length ? true : false;
...
const totalCharacters = (arr) => {
  let tot = 0;
  arr.forEach((elem)=>{
    let keysArr = ['name','spouse','children'];
    keysArr.forEach((key)=>{
      if(elem[key]){
        if(typeof elem[key] === 'string'){
          return tot ++;
        }else{
          return tot += elem[key].length;
        }
      }
    });
  });
...
const houseSize = (arr) => {
  ...
  arr.forEach((elem)=>{
    ...
    sizes.push({house:elem.house , members:tot })
  });
  return sizes;
};
...
const houseSurvivors = (arr) => {
  ...
  arr.forEach((elem)=>{
    ...
    keysArr.forEach((key)=>{
      if(elem[key]){
        if(typeof elem[key] === 'string' && !deceasedSpouses.includes(elem[key])){
          return tot ++;
    ...
    survivors.push({house:elem.house , members:tot })
  });
  return survivors;
};
